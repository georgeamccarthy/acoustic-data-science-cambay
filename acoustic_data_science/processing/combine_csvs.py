...
def combine_csvs(month):
    ...
    logging.info("Making DataFrame generator for each CSV. Name of file saved in filename column.")
...
def combine_monthly_csvs():
    ...
    for month in month_paths:
        combine_csvs(month).to_feather(os.path.join(config.raw_feathers_path,month.split('/')[-1]+'.feather'))
